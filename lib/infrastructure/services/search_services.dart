...
class SearchProviderServices {
  ///Get Posts
  Future<NearbyServiceModel> getSpecificProvider(BuildContext context,
    ...
    try {
      ...
      var request = http.Request(
          'GET',
          Uri.parse(
              BackendConfigs.apiUrlBuilder() + "service-providers/$query"));
  ...
  Future<NearbyServiceModel> getProviderByID(BuildContext context,
      {required String authToken,
      required int pageNo,
      required List query}) async {
    try {
      var headers = {
        'Authorization': 'Bearer $authToken',
        'Content-Type': 'application/json'
      ...
      var request = http.Request(
          ...
          Uri.parse(BackendConfigs.apiUrlBuilder() +
              "service-providers-by-category?category_id=${query.map((e) => e).toList().toString().replaceAll('[', "").replaceAll("]", "")}"));
